...
SEASON_POLICY_VERSION = "season-1.3"
...
AI_POLICY_BASIS_BY_STATUS = {
    "allows": "explicitly_allows_agent_submission",
    "unknown": "no_explicit_workflow_rule",
    "disallows": "disallows_agent_submission",
}
ALLOWED_TASK_TYPES = {
    "bugfix",
...
def _validate(candidate: dict[str, Any], *, as_of: date) -> tuple[date, date]:
    ...
    for field in ("url", "issue_url", "ai_policy_source_url"):
    ...
    policy_source = urlsplit(candidate["ai_policy_source_url"])
    policy_parts = policy_source.path.strip("/").split("/")
    owner, name = candidate["repository"].split("/", maxsplit=1)
    if (
        policy_source.hostname is None
        or policy_source.hostname.casefold() != "github.com"
        or policy_source.query
        or policy_source.fragment
        or "%" in policy_source.path
        or "\\" in policy_source.path
        or not policy_source.path.startswith("/")
        or policy_source.path.startswith("//")
        or policy_source.path.endswith("/")
        or any(part in {"", ".", ".."} for part in policy_parts)
        or len(policy_parts) < 5
        or policy_parts[0].casefold() != owner.casefold()
        or policy_parts[1].casefold() != name.casefold()
        or policy_parts[2] != "blob"
        or policy_parts[3].casefold() != candidate["commit_sha"].casefold()
        or not policy_parts[4]
    ):
        raise ValueError(
            "ai_policy_source_url must canonically pin a file in the candidate repository at commit_sha"
        )
    ...
    expected_basis = AI_POLICY_BASIS_BY_STATUS[candidate["ai_policy"]]
    if candidate["ai_policy_basis"] != expected_basis:
        raise ValueError("ai_policy_basis does not match ai_policy")
...
def evaluate_candidate(
    ...
    *,
    ...
    if candidate["ai_policy"] == "disallows":
        failures.append("upstream policy disallows this AI-operated contribution workflow")
    elif candidate["ai_policy"] == "unknown":
        failures.append(
            "no explicit upstream policy permits this AI-operated contribution workflow"
        )
    ...
    evidence = {
        ...
        "ai_policy": candidate["ai_policy"],
        "ai_policy_basis": candidate["ai_policy_basis"],
        "ai_policy_source_url": candidate["ai_policy_source_url"],
    }
    ...
    unresolved_gate = candidate["requires_human_attestation"] or evaluation_mode == "historical"
